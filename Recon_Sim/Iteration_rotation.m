...
%% ===== B0 map =====
addpath ('C:\Users\H-yuー32\Desktop\Junqi\MATLAB\Magnetic field');
if B0_map == 1
...
for i =1:Num_spoke
    ...
    for spoke_temp = setdiff(1:EP_angle_N,data_left1)
        ...
        Angle_detail = zeros(k_dim_x*k_dim_y,4,EP_angle_N);
...
%                             my_error_max( ix_data,iy,it_real) = max(m_error_temp);
    ...
    %     
        %                 Eval(xc_idx,k_dim_y*(ix-1)+iy) = Eval_k(G_abs,G_angle,thre_k,EP_angle_N);
                        ...
                        [Eval(xc_idx,k_dim_y*(ix-1)+iy),~,Angle_detail(k_dim_y*(ix-1)+iy,:,:)] = Eval_k5(G_abs*EP_sample_N,G_angle,thre_k,k_bound_real,1);
...
%         Eval_spoke_temp(spoke_temp,:) = reshape(sum(Eval_all,[1,2]),[1,15])'.*(B0_local(:,spoke_temp)/0.06).^(1/3); % Eval_all (144*1*15)
            Eval_spoke_temp(spoke_temp,:) = sum(Eval)/(k_dim_y*k_dim_x);
...
%% Print data
myFilePath = 'C:\Users\H-yuー32\Desktop\Eval5_least.txt'; 
myData = data_left(1,:);
exportDataToTextFile(myFilePath, myData);
